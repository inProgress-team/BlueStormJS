...
module.exports = function(name) {
    ...
    gulp.task('build@'+name, ['index.html@'+name,'assets@'+name], function() {
        if(process.env.NODE_ENV=='development') {
            var a = gulp.watch(jsFiles, function(file) {
                gulpWatch.jsFiles(file, name);
            });
            var b = gulp.watch(commonJsFilesAll, function(file) {
                gulpWatch.commonJsFilesAll(file, name);
            });
            var c = gulp.watch(templatesFiles, function(file) {
                gulp.start('html2js-watch@'+name);
            });
            var d = gulp.watch(assets, function(file) {
                gulpWatch.assets(file, name);
            });
            var e = gulp.watch(i18nFiles, ['i18n-watch@'+name]);
            var f = gulp.watch(lessFiles, ['less-watch@'+name]);
            var g = gulp.watch(htmlFile, ['index.html-watch@'+name]);
